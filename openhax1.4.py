...
Embedded docs (no ROADMAP.md on disk): EMBEDDED_ROADMAP, EMBEDDED_PIP_REQUIREMENTS,
EMBEDDED_HONEST_SCORE — Help → Roadmap / Pip / Honest score.
...
EMBEDDED_ROADMAP = f"""\
OpenHax v{__version__} — ROADMAP (embedded — no separate .md)

This is a practical middle-layer 3DS modding tool:
Open a decrypted CIA → inspect/edit RomFS/ExeFS → merge textures or inject ASM → export
a clean bundle for makerom/ctrtool (and optional LayeredFS for Luma). Encrypted CIAs:
decrypt first (GodMode9 / similar); pyctr needs readable content.

Design contract (why it stays honest)
  Ingest: pyctr deep-extracts the primary NCCH RomFS + ExeFS to a temp work tree.
  Edit:    per-file hex, Replace from disk, Revert from romfs_orig / exefs_orig snapshots.
  Export:  modification bundle — not a hidden rebuild. Full CIA/NCCH resign stays in
           external tools that already own TMD/crypto correctness; OpenHax hands off.

What ships today
────────────────────────────────────────
  • pyctr deep extraction (RomFS + ExeFS to temp; primary NCCH)
  • CIA Structure tab: glyphs, auto-expand, Refresh; double-click → hex editor
  • Safe Replace + Revert (romfs_orig / exefs_orig); right-click + toolbar Revert
  • Texture pack merge into extracted RomFS
  • Clear “Save flow” guidance + pending changes tracking
  • Modification bundle export (original.cia + romfs/ + exefs/ + README)
  • LayeredFS helper, Keystone ASM inject into ExeFS code.bin, Capstone disasm tab
  • Back to CIA binary; per-buffer Save As; Help → Honest score for a blunt snapshot

Honest limitations
────────────────────────────────────────
  • No in-app full CIA rebuild (by design — use external tools)
  • Tree uses text glyphs (works but basic; large RomFS trees need patience)
  • Save As = current buffer only (save files individually, then export bundle)
  • Hex undo is per-byte (use Revert / re-extract for whole-file experiments)
  • Cross-file search/replace is still roadmap (in-buffer search exists)

Where next (optional)
────────────────────────────────────────
  v0.4.x: progress during deep extract; optional real tree icons.
  v0.5: texture preview (Pillow), search/replace across extracted files, IPS/BPS.
  Non-goal for now: in-app resign wizard — bundle export remains the contract.

Ship note
────────────────────────────────────────
OpenHax v{__version__} is real and usable for texture mods, small edits, and code patching.
You built something honest and functional. Congrats.

Canonical documentation lives in Help → Roadmap (this text). No README drift.

Recommended pip:
  {EMBEDDED_PIP_INSTALL_LINE}
"""

EMBEDDED_HONEST_SCORE = """\
Honest Score:

Functionality: 8.5/10 — It works for real use cases (texture mods, small file edits, code.bin patching).
Stability: 7/10 — Still a bit rough (tree refresh, file replaces — use Revert from snapshot), but usable.
User Experience: 7/10 — Better than raw scripts, not as polished as big tools yet.
Uniqueness: Very high — Almost no other tool gives you this exact all-in-one Tkinter workflow.

So yes — it does its job right.
It's no longer just a proof-of-concept. It's a real tool that someone (including you) can actually use to mod 3DS games.
"""
...
class OpenHaxApp:
    def __init__(self, root):
        ...
        # Read-only copies of first extract — used by Revert (same disk as _cia_work_dir)
        self._romfs_orig_dir: Path | None = None
        self._exefs_orig_dir: Path | None = None
    ...
    def create_menu(self):
        ...
        help_menu.add_command(label="Honest score (embedded)", command=self.show_honest_score)
    ...
    def setup_cia_structure_tab(self):
        ...
        ttk.Label(outer, textvariable=self.structure_pending_var, foreground="#a05000").pack(anchor="w", pady=(0, 6))

        flow = ttk.LabelFrame(outer, text="Save flow (read this once)")
        flow.pack(fill="x", pady=(0, 8))
        ttk.Label(
            flow,
            text=(
                "Save As in the Hex tab saves only what you are viewing: full CIA blob, or one RomFS/ExeFS file. "
                "There is no single “save everything” — save each edited file, then File → Export modification bundle "
                "for repack. Revert restores a file from the first-extract snapshot (romfs_orig / exefs_orig)."
            ),
            wraplength=860,
            justify="left",
        ...
        self._romfs_menu.add_command(label="Revert to original extract…", command=self._romfs_menu_revert)
        ...
        self._exefs_menu = tk.Menu(self.tree_exefs, tearoff=0)
        self._exefs_menu.add_command(label="Open in Hex Editor", command=self._exefs_menu_open_hex)
        self._exefs_menu.add_command(label="Revert to original extract…", command=self._exefs_menu_revert)
        self.tree_exefs.bind("<Button-3>", self._on_exefs_tree_rightclick)

        row = ttk.Frame(outer)
        row.pack(fill="x", pady=(8, 0))
        ttk.Button(row, text="Refresh trees", command=self.refresh_structure_trees).pack(side="left")
        ttk.Button(row, text="Revert selected RomFS file", command=self.revert_selected_romfs_file).pack(side="left", padx=(10, 0))
        ttk.Button(row, text="Revert selected ExeFS file", command=self.revert_selected_exefs_file).pack(side="left", padx=(6, 0))
...
DOCUMENTATION
  Full ROADMAP + pip pins + Honest score are embedded in this .py (no ROADMAP.md).
  Help → Roadmap | Pip requirements | Honest score
...
CIA STRUCTURE (after opening a .cia)
  • RomFS + ExeFS extracted to a temp folder; read-only snapshots (romfs_orig / exefs_orig) for Revert.
  • Double-click a file → hex editor; right-click RomFS → Replace or Revert to original extract.
  • Right-click ExeFS → Open or Revert. Buttons: Revert selected RomFS / ExeFS file.
  • Read the “Save flow” box on this tab — Save As is per-buffer, then Export bundle.
...
""",
    ...
    def show_roadmap(self):
        ...
        win.title("OpenHax — ROADMAP (embedded)")
    ...
    def show_honest_score(self):
        win = tk.Toplevel(self.root)
        win.title("OpenHax — Honest score (embedded)")
        win.geometry("700x480")
        win.minsize(480, 320)
        t = scrolledtext.ScrolledText(win, wrap=tk.WORD, font=("Courier", 10), padx=10, pady=10)
        t.pack(fill="both", expand=True)
        t.insert("1.0", EMBEDDED_HONEST_SCORE)
        t.configure(state="disabled")
    ...
    def show_about(self):
        messagebox.showinfo(
            ...
            "Practical 3DS modding kit: hex, CIA Structure (RomFS/ExeFS), ARM/THUMB (Keystone),\n"
            "code.bin patching, textures, LayeredFS, dependency wizard.\n\n"
            "Help → Honest score (numeric snapshot); Help → Roadmap (limitations, next steps, ship note).\n\n"
            "v0.4: real, usable kit — not a concept build.",
        )
    ...
    def _cleanup_cia_session(self) -> None:
        ...
        self._extract_in_progress = False
        self.structure_pending = False
        self._romfs_orig_dir = None
        self._exefs_orig_dir = None
        try:
    ...
    def _on_deep_extract_done(self, work: Path, info: dict) -> None:
        ...
        # Snapshots for Revert (first extract only — duplicated disk under same temp work dir)
        try:
            if self._romfs_dir and self._romfs_dir.is_dir():
                orig_r = work / "romfs_orig"
                if orig_r.exists():
                    shutil.rmtree(orig_r)
                shutil.copytree(self._romfs_dir, orig_r)
                self._romfs_orig_dir = orig_r
            else:
                self._romfs_orig_dir = None
            if self._exefs_dir and self._exefs_dir.is_dir():
                orig_e = work / "exefs_orig"
                if orig_e.exists():
                    shutil.rmtree(orig_e)
                shutil.copytree(self._exefs_dir, orig_e)
                self._exefs_orig_dir = orig_e
            else:
                self._exefs_orig_dir = None
        except OSError as e:
            self._romfs_orig_dir = None
            self._exefs_orig_dir = None
            lines.append(f"Snapshot copy for Revert failed: {e}")
            self.structure_status_var.set("\n".join(lines))
    ...
    def refresh_structure_trees(self) -> None:
        ...
        if self._romfs_dir and self._romfs_dir.is_dir():
            ...
            try:
                self.tree_romfs.item("", open=True)
            except Exception:
                pass
        if self._exefs_dir and self._exefs_dir.is_dir():
            for p in sorted(self._exefs_dir.iterdir(), key=lambda x: x.name.lower()):
                if p.is_file():
                    try:
                        ...
                        sz = "?"
                    self.tree_exefs.insert(
                        "",
                        "end",
                        iid=f"exefs:{p.name}",
                        text=f"\U0001f4c4 {p.name}",
                        values=(sz,),
                    )
    ...
    def _fill_romfs_tree(self, base: Path, parent: str = "") -> None:
        ...
        for p in items:
            ...
            if p.is_dir():
                ...
                self.tree_romfs.item(node, open=True)
    ...
    def _romfs_menu_revert(self) -> None:
        iid = getattr(self, "_romfs_menu_iid", None)
        if iid:
            self._revert_romfs_iid(iid)

    def revert_selected_romfs_file(self) -> None:
        sel = self.tree_romfs.selection()
        if not sel:
            messagebox.showinfo("Revert", "Select a RomFS file in the tree.")
            return
        self._revert_romfs_iid(sel[0])

    def _revert_romfs_iid(self, iid: str) -> None:
        if not iid.startswith("romfs:") or not self._romfs_dir or not self._romfs_orig_dir:
            messagebox.showwarning("Revert", "No snapshot — open a decrypted CIA and wait for extract.")
            return
        rel = iid[6:]
        dest = self._romfs_dir / rel
        src = self._romfs_orig_dir / rel
        if not dest.is_file():
            messagebox.showinfo("Revert", "Select a file row, not a folder.")
            return
        if not src.is_file():
            messagebox.showerror("Revert", "Original snapshot missing. Re-open the CIA.")
            return
        try:
            shutil.copy2(src, dest)
            self.structure_pending = True
            self.structure_pending_var.set(
                "Changes pending: file reverted from snapshot — export bundle still needed for CIA rebuild."
            )
            ctx = self.hex_context
            if ctx.get("kind") == "romfs" and ctx.get("rel") == rel:
                self._load_file_into_hex(dest, "romfs", rel)
            self.refresh_structure_trees()
            self.refresh_status_bar()
            messagebox.showinfo("Revert", f"Restored from first extract:\n{dest}")
        except OSError as e:
            messagebox.showerror("Revert", str(e))

    def _on_exefs_tree_rightclick(self, event: tk.Event) -> None:
        iid = self.tree_exefs.identify_row(event.y)
        if not iid:
            return
        self.tree_exefs.selection_set(iid)
        self._exefs_menu_iid = iid
        try:
            self._exefs_menu.tk_popup(event.x_root, event.y_root)
        finally:
            self._exefs_menu.grab_release()

    def _exefs_menu_open_hex(self) -> None:
        iid = getattr(self, "_exefs_menu_iid", None)
        if iid and iid.startswith("exefs:") and self._exefs_dir:
            name = iid[6:]
            full = self._exefs_dir / name
            if full.is_file():
                self._load_file_into_hex(full, "exefs", name)

    def _exefs_menu_revert(self) -> None:
        iid = getattr(self, "_exefs_menu_iid", None)
        if iid:
            self._revert_exefs_iid(iid)

    def revert_selected_exefs_file(self) -> None:
        sel = self.tree_exefs.selection()
        if not sel:
            messagebox.showinfo("Revert", "Select an ExeFS entry in the tree.")
            return
        self._revert_exefs_iid(sel[0])

    def _revert_exefs_iid(self, iid: str) -> None:
        if not iid.startswith("exefs:") or not self._exefs_dir or not self._exefs_orig_dir:
            messagebox.showwarning("Revert", "No snapshot — open a decrypted CIA and wait for extract.")
            return
        name = iid[6:]
        dest = self._exefs_dir / name
        src = self._exefs_orig_dir / name
        if not dest.is_file() or not src.is_file():
            messagebox.showerror("Revert", "Could not revert this entry.")
            return
        try:
            shutil.copy2(src, dest)
            self.structure_pending = True
            self.structure_pending_var.set(
                "Changes pending: ExeFS file reverted from snapshot — export bundle still needed for CIA rebuild."
            )
            ctx = self.hex_context
            if ctx.get("kind") == "exefs" and ctx.get("rel") == name:
                self._load_file_into_hex(dest, "exefs", name)
            self.refresh_structure_trees()
            self.refresh_status_bar()
            messagebox.showinfo("Revert", f"Restored from first extract:\n{dest}")
        except OSError as e:
