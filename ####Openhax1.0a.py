...
"""
OpenHax — 3DS Hacking Suite (v1.0-mvp by a.c)
...
__version__ = "1.0-mvp"
PYTHON_TARGET = (3, 14)
APP_STATE_PATH = Path.home() / ".openhax_state.json"
...
def require_runtime() -> None:
    if sys.version_info < PYTHON_TARGET:
        ...
        raise RuntimeError(f"OpenHax requires Python {wanted}+ (got {got}).")
...
class HexEditorFrame(ttk.Frame):
    ...
    def setup_ui(self):
        ...
        ttk.Button(toolbar2, text="Replace", command=self.replace_current).pack(side='left', padx=(4, 0))
        ttk.Button(toolbar2, text="Add Bookmark", command=self.add_bookmark).pack(side='left', padx=(12, 0))
        ttk.Button(toolbar2, text="Next Bookmark", command=self.next_bookmark).pack(side='left', padx=(4, 0))
    ...
    def replace_current(self):
        ...
        raw = self.search_var.get().strip()
        if not raw:
            messagebox.showwarning("Replace", "Enter a search pattern first.")
            return
        mode = self.search_mode_var.get().lower()
        if mode == "ascii":
            needle = raw.encode("utf-8", errors="replace")
            repl_s = simpledialog.askstring("Replace", "Replacement ASCII text:", parent=self)
            if repl_s is None:
                return
            repl = repl_s.encode("utf-8", errors="replace")
        else:
            needle = self._parse_search_hex(raw)
            if needle is None:
                messagebox.showerror("Replace", "Invalid hex search value.")
                return
            repl_raw = simpledialog.askstring(
                "Replace",
                "Replacement hex bytes (eg: DE AD BE EF):",
                parent=self,
            )
            if repl_raw is None:
                return
            repl = self._parse_search_hex(repl_raw.strip())
            if repl is None:
                messagebox.showerror("Replace", "Invalid replacement hex value.")
                return
        if not needle:
            return
        data = bytes(self.cia_data)
        pos = data.find(needle, self.cursor_pos)
        if pos < 0:
            messagebox.showinfo("Replace", "Pattern not found from current cursor.")
            return
        # Save undo checkpoint for each replaced byte.
        end = pos + len(needle)
        old = self.cia_data[pos:end]
        self.cia_data[pos:end] = repl
        for i, ob in enumerate(old):
            self._undo_stack.append((pos + i, int(ob)))
    ...
    def add_bookmark(self):
        if self.cia_data is None:
            return
        if self.cursor_pos not in self._bookmarks:
            self._bookmarks.append(self.cursor_pos)
            self._bookmarks.sort()
        messagebox.showinfo("Bookmark", f"Saved bookmark at 0x{self.cursor_pos:08X}")

    def next_bookmark(self):
        if not self._bookmarks:
            messagebox.showinfo("Bookmark", "No bookmarks yet.")
            return
        target = None
        for b in self._bookmarks:
            if b > self.cursor_pos:
                ...
                break
        if target is None:
            target = self._bookmarks[0]
        self.cursor_pos = target
        self.cursor_nibble = 0
        target_line = target // self.bytes_per_row
        if target_line < self.top_line_index or target_line >= self.top_line_index + self.visible_lines:
            self.top_line_index = max(0, target_line - self.visible_lines // 2)
            self.clamp_scroll()
            self.update_scrollbar()
...
class OpenHaxApp:
    def __init__(self, root):
        ...
        self.custom_makerom_path: str = ""
        self.custom_ctrtool_path: str = ""
        self._menu_recent: tk.Menu | None = None
    ...
    def _load_app_state(self) -> None:
        self.recent_files = []
        self.custom_makerom_path = ""
        self.custom_ctrtool_path = ""
        try:
            if APP_STATE_PATH.is_file():
                state = json.loads(APP_STATE_PATH.read_text(encoding="utf-8"))
                recents = state.get("recent_files", [])
                if isinstance(recents, list):
                    self.recent_files = [str(p) for p in recents if isinstance(p, str) and Path(p).exists()][:8]
                self.custom_makerom_path = str(state.get("makerom_path", "") or "")
                self.custom_ctrtool_path = str(state.get("ctrtool_path", "") or "")
        except Exception:
            self.recent_files = []
    ...
    def _save_app_state(self) -> None:
        try:
            payload = {
                ...
                "makerom_path": self.custom_makerom_path,
                "ctrtool_path": self.custom_ctrtool_path,
    ...
    def _show_error(self, title: str, msg: str) -> None:
        self._set_status(f"{title}: {msg[:120]}")
        messagebox.showerror(title, msg)

    def _run_with_progress(self, start_msg: str, done_msg: str, fn, on_ok=None, on_err=None) -> None:
        self._set_status(start_msg)

        def worker() -> None:
            try:
                res = fn()
                self.root.after(0, lambda: _done_ok(res))
            except Exception as e:
                self.root.after(0, lambda: _done_err(str(e)))

        def _done_ok(res) -> None:
            self._set_status(done_msg)
            if on_ok:
                on_ok(res)

        def _done_err(err: str) -> None:
            if on_err:
                on_err(err)
            else:
                self._show_error("Operation failed", err)
    ...
    def create_menu(self):
        ...
        self._menu_recent = tk.Menu(file_menu, tearoff=0)
        file_menu.add_cascade(label="Open Recent", menu=self._menu_recent)
        ...
        file_menu.add_command(label="One-click repack (makerom/ctrtool)…", command=self.repack_cia)
        file_menu.add_separator()
        file_menu.add_command(label="Exit", command=self.root.quit)
        menubar.add_cascade(label="File", menu=file_menu)
        
        tools_menu = tk.Menu(menubar, tearoff=0)
        tools_menu.add_command(label="RomFS diff (baseline vs extracted)…", command=self.show_romfs_diff)
        tools_menu.add_command(label="Export LayeredFS from extracted RomFS…", command=self.export_layeredfs_from_staged)
        tools_menu.add_command(label="Set makerom path…", command=self.set_makerom_path)
        tools_menu.add_command(label="Set ctrtool path…", command=self.set_ctrtool_path)
    ...
    def _refresh_recent_menu(self) -> None:
        if not self._menu_recent:
            return
        self._menu_recent.delete(0, tk.END)
        if not self.recent_files:
            self._menu_recent.add_command(label="(none)", state="disabled")
            return
        for p in self.recent_files:
            self._menu_recent.add_command(label=p, command=lambda pp=p: self.open_cia(pp))
...
""",
    ...
    def open_cia(self, filepath: str | None = None):
        if not filepath:
    ...
    def _on_deep_extract_done(self, work: Path, info: dict) -> None:
        ...
        if self._romfs_dir:
            ...
            baseline = work / "romfs_baseline"
            if not baseline.exists():
                try:
                    shutil.copytree(self._romfs_dir, baseline)
                except Exception:
                    pass
    ...
    def _detect_tool(self, name: str, custom_path: str = "") -> str | None:
        if custom_path and Path(custom_path).is_file():
            return custom_path
        auto = shutil.which(name)
        return auto

    def set_makerom_path(self) -> None:
        p = filedialog.askopenfilename(title="Select makerom executable")
        if not p:
            return
        self.custom_makerom_path = p
        self._save_app_state()
        self._set_status(f"makerom path set: {p}")

    def set_ctrtool_path(self) -> None:
        p = filedialog.askopenfilename(title="Select ctrtool executable")
        if not p:
            return
        self.custom_ctrtool_path = p
        self._save_app_state()
        self._set_status(f"ctrtool path set: {p}")

    def show_romfs_diff(self) -> None:
        if not self._romfs_dir or not self._romfs_dir.is_dir():
            messagebox.showwarning("RomFS diff", "Load and extract a decrypted CIA first.")
            return
        if not self._cia_work_dir:
            messagebox.showwarning("RomFS diff", "No baseline workspace available.")
            return
        baseline = self._cia_work_dir / "romfs_baseline"
        if not baseline.is_dir():
            # Snapshot current extracted state as baseline if not present yet.
            try:
                shutil.copytree(self._romfs_dir, baseline)
            except Exception:
                pass
        lines: list[str] = []
        if baseline.is_dir():
            base_files = {str(p.relative_to(baseline)): p for p in baseline.rglob("*") if p.is_file()}
            cur_files = {str(p.relative_to(self._romfs_dir)): p for p in self._romfs_dir.rglob("*") if p.is_file()}
            for rel in sorted(cur_files.keys() | base_files.keys()):
                ...
                if b is None:
                    ...
                    try:
                        if b.stat().st_size != c.stat().st_size or b.read_bytes() != c.read_bytes():
        ...
        win = tk.Toplevel(self.root)
        win.title("RomFS diff")
        win.geometry("760x500")
        txt = scrolledtext.ScrolledText(win, wrap=tk.NONE, font=("Courier", 10))
        txt.pack(fill="both", expand=True)
        txt.insert("1.0", "\n".join(lines) + "\n")
        txt.configure(state="disabled")
        self._set_status(f"RomFS diff: {max(0, len(lines) - (1 if lines == ['No differences found.'] else 0))} change(s)")

    def export_layeredfs_from_staged(self) -> None:
        if not self._romfs_dir or not self._romfs_dir.is_dir():
            messagebox.showwarning("LayeredFS", "No extracted RomFS available.")
            return
        tid = (self.txt_title_id.get() or "").strip().upper()
        if not tid:
            messagebox.showwarning("LayeredFS", "Title ID is empty. Open a CIA with metadata first.")
            return
        out_base = filedialog.askdirectory(title="Select folder to create luma/titles/... under")
        if not out_base:
            return
        dest = Path(out_base) / "luma" / "titles" / tid / "romfs"
        try:
            if dest.exists():
                shutil.rmtree(dest)
            shutil.copytree(self._romfs_dir, dest)
            messagebox.showinfo("LayeredFS", f"Exported staged RomFS to:\n{dest}")
            self._set_status(f"LayeredFS export complete: {dest}")
        except Exception as e:
            self._show_error("LayeredFS", str(e))

    def repack_cia(self) -> None:
        if not self.loaded_cia_path or not str(self.loaded_cia_path).lower().endswith(".cia"):
            messagebox.showerror("Repack", "Open a .cia file first.")
            return
        makerom = self._detect_tool("makerom", self.custom_makerom_path)
        ctrtool = self._detect_tool("ctrtool", self.custom_ctrtool_path)
        if not makerom or not ctrtool:
            msg = (
                "One-click repack requires makerom and ctrtool.\n\n"
                f"makerom: {'FOUND' if makerom else 'missing'}\n"
                f"ctrtool: {'FOUND' if ctrtool else 'missing'}\n\n"
                "Set custom paths in Tools menu or install tools in PATH.\n"
                "Fallback: File → Export CIA modification bundle."
            )
            messagebox.showwarning("Repack tools missing", msg)
            return
        if not self._romfs_dir or not self._exefs_dir:
            messagebox.showwarning("Repack", "Extracted RomFS/ExeFS not available. Open a decrypted CIA and wait for extraction.")
            return
        out = filedialog.asksaveasfilename(
            title="Save rebuilt CIA (toolchain output path)",
            initialfile=f"{Path(self.loaded_cia_path).stem}_repacked.cia",
        ...
        log_path = Path(out).with_suffix(".repack.log.txt")
        ...
        def _job():
            ...
            # We intentionally run a minimal makerom command as a toolchain probe/log generator.
            # Users can then refine with title keys/content specifics externally.
            proc = subprocess.run(cmd, capture_output=True, text=True)
            log = [
                "OpenHax repack attempt",
                f"makerom: {makerom}",
                f"ctrtool: {ctrtool}",
                f"command: {' '.join(cmd)}",
            ...
            if proc.returncode != 0:
                raise RuntimeError(
                    "makerom returned a non-zero exit code.\n"
                    f"See log: {log_path}\n\n"
                    "Fallback: use Export modification bundle and your existing repack pipeline."
                )
            return out

        def _ok(_res):
            messagebox.showinfo("Repack", f"Repack command completed.\nOutput/log:\n{out}\n{log_path}")

        self._run_with_progress("Running one-click repack…", "Repack finished.", _job, on_ok=_ok)
    ...
    def __init__(self, root, on_launch):
        ...
        self.dependencies = {
            "pyctr": {
                "pip_name": "pyctr",
                "import_name": "pyctr",
                "desc": "Required — CIA/NCCH parsing, RomFS/ExeFS (OpenHax v1.0-mvp)",
                "installed": False,
            },
            "keystone-engine": {
...
if __name__ == "__main__":
    ...
    def launch_app():
        if not _dep_pyctr():
            messagebox.showerror(
                "pyctr required",
                "OpenHax v1.0-mvp requires pyctr for CIA metadata and NCCH tools.\n\n"
